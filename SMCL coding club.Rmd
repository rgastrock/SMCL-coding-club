...
We do a Bayesian analysis on the extent of learning (block 3 from above, trials 76-90):
...
There are two kinds no-cursor reach blocks: with and without strategy. Without strategy is supposed to measure classic implicit adaptation, while with strategy is supposed to cpature both implicit and explicit adapation.
...
We will use all of the no-cursor reach deviations. Baseline both the with and without strategy no-cursor using the aligned data. Then plot te group averages with confidence intervals, and the individual data.

```{r}
plotNoCursors()
```
So it seems there are is an effect of using a strategy in the no-cursor reaches, except in the control group group. Let's test this. First with an ANOVA, with post-hoc follow-ups:
...
Comparing the handview and cursorjump group with-strategy no cursor reaches with those in the control group, shows a difference in both case. This means that in these two groups there were explicit strategies.
...
The handview group has a difference in the without strategy no-cursor reaches with both the control group and the cursorjump group. This means that implicit adaptation is lower in thsi group as compared to others.
